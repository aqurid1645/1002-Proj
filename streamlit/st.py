...
st.title('Inflation')
data = st.radio("Choose a dataset to view", ("CPI", "Import Price"))
...
st.sidebar.markdown('Range of year')
xasix=st.sidebar.slider('Year',min_value=2017,max_value=2021,value=[2017,2021])
if data=='CPI':
   st.title('CPI')
   ...
   st.write('From the graph shown, we can see that majority of categories have percentage increase in CPI from 2020 especially in Transport, which could be COE being the main culprit')
elif data=='Import Price':
   ...
   'increasing unemplyment rate which means fewer goods and services produced and thus higher prices')
   st.download_button(
